  ...
  async *chat(
    ...
    try {
      const stream = await this.client.chat.completions.create({
        model: this.model || "accounts/fireworks/models/llama-v3p1-8b-instruct",
        ...
        if (delta.tool_calls) {
          for (const tc of delta.tool_calls) {
            ...
            if (!toolCalls.has(index)) {
              toolCalls.set(index, { id: "", name: "", arguments: "", started: false });
            }
        ...
        if (choice.finish_reason === "tool_calls") {
          for (const [, tc] of toolCalls) {
            try {
              ...
              yield {
                type: "error",
                error: `Fireworks AI: Failed to parse tool arguments for ${tc.name}`,
              };
            }
          }
        }
      ...
      yield {
        type: "error",
        error: `Fireworks AI API error: ${err.message}`,
      };
    }
  }
