...
const ToolMessage = memo(({ tool, settings, agentInfo }: ToolMessageProps) => {
  ...
  const resolveLabel = () => {
    ...
    if (tool?.name === 'file_read' || tool?.name === 'file_edit' || tool?.name === 'file_write') {
      return tool?.args?.path?.split(/[/\\]/).pop() || tool?.args?.path || tool?.name
    }
    if (tool?.name === 'shell') {
      return tool?.command || tool?.args?.command || tool?.name
    }
    return tool?.name
  }
